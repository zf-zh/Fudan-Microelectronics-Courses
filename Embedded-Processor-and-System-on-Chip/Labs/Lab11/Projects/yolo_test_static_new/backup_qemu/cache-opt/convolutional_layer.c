...
float*** malloc3DArray(int w, int h, int c, float* in) {
    float*** array = (float***)malloc(c * sizeof(float**));
    for (int k = 0; k < c; k++) {
        array[k] = (float**)malloc(h * sizeof(float*));
        for (int j = 0; j < h; j++) {
            array[k][j] = (float*)malloc(w * sizeof(float));
            for(int i = 0; i < w; i++){
            if(in != NULL) array[k][j][i] = in[k*h*w+j*w+i];
            else array[k][j][i] = 0;
            }
        }
    }
...
float*** malloc3DArray_pad(int w, int h, int c, float* in) {
    float*** array = (float***)malloc(c * sizeof(float**));
    for (int k = 0; k < c; k++) {
        array[k] = (float**)malloc(h * sizeof(float*));
        for (int j = 0; j < h; j++) {
            ...
            if(in != NULL) 
            {
            	...
            	array[k][j][i] = in[k*(h-2)*(w-2)+(j-1)*(w-2)+(i-1)];
            	else array[k][j][i] = 0;
            }
            else array[k][j][i] = 0;
            }
...
float**** malloc4DArray(int w, int h, int c, int n,float* in) {
    float**** array = (float****)malloc(n * sizeof(float***));
    for (int z = 0; z < n; z++) {
        array[z] = (float***)malloc(c * sizeof(float**));
        for (int k = 0; k < c; k++) {
            array[z][k] = (float**)malloc(h * sizeof(float*));
            for(int j = 0; j < h; j++){
            	array[z][k][j] = (float*)malloc(w*sizeof(float));
            	for(int i = 0; i < w; i++){
            		if(in != NULL) 
            		{
            			array[z][k][j][i]=in[z*w*h*c+k*h*w+j*w+i];
            		}
            		else array[z][k][j][i] = 0;
            	}
            }
        }
    }
...
void free_3D(float ***p, int w_o, int h_o, int c_o) {
    if (p != NULL) {
        for (int c = 0; c < c_o; c++) {
            if (p[c] != NULL) {
                for (int h = 0; h < h_o; h++) {
...
void free_4D(float ****p, int w_o, int h_o, int c_o,int d_o) {
    if (p != NULL) {
        for (int d = 0; d < d_o; d++) {
            if (p[d] != NULL) {
                for (int c = 0; c < c_o; c++) {
...
void convolutional_compute(layer_params para, float* input, float* weight, float* output)
{
    ...
    long int cycle_end, instret_end;
    ...
    int w_i,h_i;
    int w_o,h_o,n_o;
    int kernel_n = para.kernel_n;
    
    float*** array_in = NULL;
    float*** array_out = NULL;
    float**** kernel_in = NULL;
    ...
    w_o = (w_i-kernel_size)/stride +1;
    h_o = (h_i-kernel_size)/stride +1;
    n_o = kernel_n;
    
    if(para.pad == 0) array_in = malloc3DArray(w_i,h_i,c_i,input);
    else array_in = malloc3DArray_pad(w_i,h_i,c_i,input);
    
    array_out = malloc3DArray(w_o,h_o,kernel_n,NULL);
    
    kernel_in = malloc4DArray(kernel_size,kernel_size,c_i,kernel_n,weight);
    		...
    		for(int j = 0; j < h_o; j++){
    			for( int i = 0; i < w_o; i++)
    			{
    			 	for( int y = 0; y < kernel_size; y++){
    			 		...
    			 		{
    			 			array_out[l][j][i] += array_in[k][j*stride+y][i*stride+x]*kernel_in[l][k][y][x];
    			 		}
    			 	}
    			}
    		}
    ...
    for(int k = 0; k < kernel_n ; k++)
    {
    	...
    	{
            for (int i = 0; i < w_o ; i++){
            output[k*(h_o)*(w_o)+j*(w_o)+i] = array_out[k][j][i];
            }
    	}
    }
    free_3D(array_in,w_i,h_i,c_i);
    free_3D(array_out,w_o,h_o,n_o);
    free_4D(kernel_in,kernel_size,kernel_size,c_i,kernel_n);
