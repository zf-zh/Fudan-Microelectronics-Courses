...
void convolutional_compute(layer_params para, float* input, float* weight, float* output)
{
    ...
    long int cycle_begin, instret_begin;
    long int cycles, instrets;
    ...
    float *conv_tmp;
    conv_tmp = (float *)malloc(sizeof(float)*4);
    ...
    tmp_idx = 0;
    remain_slice = conv_size_c%4;
    tmp_conv_size_c = remain_slice==0 ? conv_size_c : conv_size_c + 4 - remain_slice; // 4x ch for slices
    size_of_weight = tmp_conv_size_c*kernel_size*kernel_size*padding_size_c; // tmp_conv_size_c-output ch, padding_size_c-input ch
    tmp_weight = (float *)malloc(sizeof(float)*size_of_weight);

    for (loop_count = 0; loop_count < tmp_conv_size_c/4; loop_count++)
        for (c_k=0; c_k<padding_size_c; c_k++)
            for(c_j=0; c_j<kernel_size; c_j++)
                for(c_i=0; c_i<kernel_size; c_i++)
                    for(k=0; k<4; k++)
                    {
                        if (4*loop_count+k < conv_size_c)
                            tmp_weight[tmp_idx++] = weight[c_i + c_j*kernel_size + c_k*kernel_size*kernel_size + \
                            (4*loop_count+k)*kernel_size*kernel_size*padding_size_c];
                        else
                            tmp_weight[tmp_idx++] = 0.0;
                    }

    int input_idx;
    int weight_idx;

    for (k=0; k<tmp_conv_size_c/4; k++)
            ...
            {   
                ...
                for (c_k=0; c_k<padding_size_c; c_k++) 
                    for(c_j=0; c_j<kernel_size; c_j++)
                        for(c_i=0; c_i<kernel_size; c_i++)
                        {
                            input_idx = (i*stride + c_i) + (j*stride*padding_size_w + padding_size_w * c_j) + \
                                        (padding_size_w * padding_size_h * c_k);
                            weight_idx = 4*(c_i + c_j*kernel_size + c_k*kernel_size*kernel_size + k*kernel_size*kernel_size*padding_size_c);
                            
                            conv_tmp[0] += tmp_pad[input_idx]*tmp_weight[weight_idx];
                            conv_tmp[1] += tmp_pad[input_idx]*tmp_weight[weight_idx+1];
                            conv_tmp[2] += tmp_pad[input_idx]*tmp_weight[weight_idx+2];                                                                  
                            conv_tmp[3] += tmp_pad[input_idx]*tmp_weight[weight_idx+3];
                            
                        }
                output[i + j*conv_size_w + (4*k)*conv_size_h*conv_size_w] = conv_tmp[0]; 
                if (4*k+1 < conv_size_c)
                    output[i + j*conv_size_w + (4*k+1)*conv_size_h*conv_size_w] = conv_tmp[1];
                if (4*k+2 < conv_size_c)
                    output[i + j*conv_size_w + (4*k+2)*conv_size_h*conv_size_w] = conv_tmp[2];
                if (4*k+3 < conv_size_c)
                    output[i + j*conv_size_w + (4*k+3)*conv_size_h*conv_size_w] = conv_tmp[3];
            }
