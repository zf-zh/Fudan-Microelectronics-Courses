...
static void put_pixel(unsigned char* d, int w, int h, int x, int y,
...
static void fill_rect(unsigned char* d, int w, int h, int x1, int y1, int x2, int y2,
                      unsigned char r, unsigned char g, unsigned char b)
{
    for (int y = y1; y <= y2; y++)
        for (int x = x1; x <= x2; x++)
            put_pixel(d, w, h, x, y, r, g, b);
}

static void draw_text(unsigned char* d, int w, int h, int x, int y, const char* s, int scale,
                      unsigned char r, unsigned char g, unsigned char b)
{
    static const unsigned char DOT[8] = {0,0,0,0,0,0x18,0x18,0};
    for (; *s; s++, x += 8 * scale) {
        const unsigned char* rows;
        if      (*s >= '0' && *s <= '9') rows = DIGITS[*s - '0'];
        else if (*s == '.')              rows = DOT;
        else continue;                       /* space / anything else: just advance */
        for (int ry = 0; ry < 8; ry++)
            for (int rx = 0; rx < 8; rx++)
                if ((rows[ry] >> rx) & 1)
                    for (int sy = 0; sy < scale; sy++)
                        for (int sx = 0; sx < scale; sx++)
                            put_pixel(d, w, h, x + rx*scale + sx, y + ry*scale + sy, r, g, b);
    }
}
...
void GenerateOutputPicture(struct image_data *input_img, struct image_data *output_img, const char* filename)
{
    ...
    if (output_bboxes->data != NULL)
    {
        ...
        {   
            ...
            static const char* GESTURE[6] = {"THUMB", "ONE", "TWO", "THREE", "FOUR", "FIVE"};
            int   cls  = (class >= 0 && class < 6) ? class : 0;
            float conf = output_bboxes->data[i*6+4];

            snprintf(classname, JPG_CLASS_LENGTH, "%d %.2f", cls, conf);

            int scale = (in_w >= 320) ? 2 : 1;
            int th    = 8 * scale;
            int tw    = (int)strlen(classname) * 8 * scale;

            int tx = x1;
            int ty = y1 - th - 2;
            if (ty < 0)         ty = y1 + 2;
            if (tx + tw > in_w) tx = in_w - tw;
            if (tx < 0)         tx = 0;

            // End

            fill_rect(input_char, in_w, in_h, tx-1, ty-1, tx+tw, ty+th, 0, 0, 0);   /* dark backing */
            draw_text(input_char, in_w, in_h, tx, ty, classname, scale, 0, 255, 125); /* box-green text */
...
struct image_data* non_max_suppression(struct image_data *input_img, float conf_thres, float nms_thres)
{
    ...
    */
    ...
    int input_W = input_img->shape[1];
    int input_C = input_img->shape[2];

    int output_box_count = 0;
    float* output_data = NULL;

    const int S = 6;
    float* src = input_img->data;

    // step1: drop boxes below the confidence threshold
    float* cand = (float*)malloc(sizeof(float) * input_H * S);
    int n = 0;
    for (int i = 0; i < input_H; i++) {
        if (src[i*S + 4] >= conf_thres) {
            memcpy(&cand[n*S], &src[i*S], sizeof(float) * S);
            n++;
        }
    }

    if (n > 0) {
        // step2: sort all survivors by confidence, descending
        confQuickSort(cand, 0, n - 1);

        int* dead = (int*)calloc(n, sizeof(int));
        int* keep = (int*)malloc(sizeof(int) * n);

        // step3-7: greedy suppression
        for (int i = 0; i < n; i++) {
            if (dead[i]) continue;
            keep[output_box_count++] = i;
            for (int j = i + 1; j < n; j++) {
                if (dead[j]) continue;
                if ((int)cand[j*S + 5] != (int)cand[i*S + 5]) continue;
                if (bbox_iou(&cand[i*S], &cand[j*S], 0) > nms_thres) dead[j] = 1;
            }
        }

        // step8: pack kept boxes into the output
        output_data = (float*)malloc(sizeof(float) * output_box_count * S);
        for (int i = 0; i < output_box_count; i++)
            memcpy(&output_data[i*S], &cand[keep[i]*S], sizeof(float) * S);
