...
static void postprocess(void *sess, const char *filename_prefix) {
    ...
    for (int i = 0; i < output_num; i++) {
        ...
        /************************genjpg**********************/
        struct image_data* input_img = calloc(1, sizeof(struct image_data));
        input_img->data = input->data;        // NOLINT
        chw2hwc(input_img->data, 3, RESIZE_HEIGHT, RESIZE_WIDTH);
        // hwc, h=RESIZE_HEIGHT w=RESIZE_WIDTH c=3
        input_img->data = (float*)malloc(sizeof(float) * (RESIZE_HEIGHT * RESIZE_WIDTH * 3)); 

        input_img->shape = (uint32_t*)malloc(sizeof(uint32_t) * 3);                        // NOLINT
        input_img->shape[0] = RESIZE_HEIGHT;
        input_img->shape[1] = RESIZE_WIDTH;
        input_img->shape[2] = 3;
        input_img->dim = 3;

        struct image_data* output_img = calloc(1, sizeof(struct image_data));
        output_img->data = (float*)foutput->data;
        chw2hwc(output_img->data, 11, 20, 20);
        // hwc, h=20 w=20 c=11
        output_img->shape = (uint32_t*)malloc(sizeof(uint32_t) * 3);
        output_img->shape[0] = 20;
        output_img->shape[1] = 20;
        output_img->shape[2] = 11;
        output_img->dim = 3;

        char jpgname[FILE_LENGTH] = {0};
        snprintf(jpgname, FILE_LENGTH, "%s_output%u", filename_prefix, i);
        GenerateOutputPicture(input_img, output_img, jpgname);
        
        free_image_data(input_img);
